...
4. **App tokens only.** Colors, radius, shadows, type from Star Admin 2 Pro (`codebases/AdLauncher/app/globals.css`). No JetBrains Mono dark cyber theme (that is `/pm-architecture-diagram`). No generic SaaS purple unrelated to the app.
...
| Dark cyber infra topology | `/pm-architecture-diagram` |
| Hi-fi prototype / PPT / animation | `/huashu-design` |
| Durable AS-IS overwrite | `/pm-vault-note-update` after the canvas is agreed |
