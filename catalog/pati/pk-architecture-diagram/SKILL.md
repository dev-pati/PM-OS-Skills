...
- `references/cocoon-dark-theme-svg.md` — Cocoon AI dark-themed HTML+SVG architecture-diagram style guide (color palette, spacing rules, export toolbar). Use as a visual-style reference when a diagram needs the dark cyberpunk-infra look instead of Archify's default theme; `references/cocoon-resources/` has the template HTML.
- `references/bpmn-plantuml.md` — PlantUML-syntax BPMN/EIP/Lean-Mapping stencil reference for business-process, workflow-automation, approval-chain, and value-stream diagrams. Use this instead of the JSON-schema path above when the ask is specifically a PlantUML/BPMN diagram (rendered via the docu.md Markdown Viewer convention), not an Archify HTML artifact. `references/bpmn-examples/` has worked examples.
