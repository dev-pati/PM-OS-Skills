---
name: pk-ui-decide
description: "Use when a UI task needs design decisions before implementation: infer the brief, choose a visual direction, generate a design system, select palette/type/layout/motion, or review design and accessibility guidance. Combines UI/UX Pro Max's searchable catalog with Frontend Design's subject-specific anti-template process. For implementation and live-browser verification, use pk-ui-verify."
license: MIT
metadata:
  source-repo: https://github.com/nextlevelbuilder/ui-ux-pro-max-skill
  source-path: src/ui-ux-pro-max
  source-commit: "4aad058"
  source-fetched: "2026-09-06"
  adapted: "true"
  installer: npx skills add ... --skill ui-ux-pro-max
  runtime: python3 (stdlib only); data queried via scripts/search.py, never read whole
---

# PATI UI Decide

Choose a defensible visual direction before code. The skill combines the UI/UX
Pro Max searchable catalog with Frontend Design's brief-first, distinctive
aesthetic process.

Searchable local guidance: 79 searchable styles (50 active), 192 product
palettes and exact reasoning profiles, 74 font pairings, 119 UX guidelines,
105 curated icons, 17 GSAP presets, 25 chart types, and 22 technology stacks.

## When to Apply

Use this skill when the task involves **UI structure, visual design decisions,
interaction patterns, or user experience quality control**: designing new
pages, choosing color/typography/spacing/layout systems, selecting navigation
or animation patterns, or reviewing UI for UX/accessibility/consistency.

Skip it for pure backend logic, API/database design, non-visual performance work, infrastructure/DevOps, or non-visual scripts — unless the task changes how something **looks, feels, moves, or is interacted with**.

## Boundary with `pk-ui-verify`

- `pk-ui-decide` answers **what should this interface be and why?**
- `pk-ui-verify` answers **did the implemented interface actually work in a real browser?**
- For a redesign, use both in sequence: decide first, implement, verify last.
- When the direction is already fixed and the request is a local UI fix, skip
  this skill and use `pk-ui-verify` directly.
...
Resolve the installed skill directory once, then call its script. The PATI
workspace exposes it at `03_agents/skills/_flat/pk-ui-decide`; Codex also
links it at `~/.codex/skills/pk-ui-decide`:

```bash
PATI_UI_DECIDE="${CODEX_HOME:-$HOME/.codex}/skills/pk-ui-decide"
python3 "$PATI_UI_DECIDE/scripts/search.py" "<query>" --domain <domain>
```

If `python3` is unavailable, use `python`, then `py -3`. Requires Python 3.x;
the search runtime uses only the standard library.
...
```bash
python3 "$PATI_UI_DECIDE/scripts/search.py" "<product_type> <industry> <keywords>" --design-system [-p "Project Name"]
```
...
**Example:**
```bash
python3 "$PATI_UI_DECIDE/scripts/search.py" "beauty spa wellness service" --design-system -p "Serenity Spa"
```
...
```bash
python3 "$PATI_UI_DECIDE/scripts/search.py" "<query>" --design-system --persist -p "Project Name" --output-dir "<project-root>"
```
...
```bash
python3 "$PATI_UI_DECIDE/scripts/search.py" "<query>" --design-system --variance <1-10> --motion <1-10> --density <1-10>
```
...
**Example:**
```bash
python3 "$PATI_UI_DECIDE/scripts/search.py" "internal analytics dashboard" --design-system --variance 8 --motion 7 --density 8 -p "Ops Console"
```

### Step 3: Supplement with Detailed Searches (as needed)

```bash
python3 "$PATI_UI_DECIDE/scripts/search.py" "<keyword>" --domain <domain> [-n <max_results>]
```
...
```bash
python3 "$PATI_UI_DECIDE/scripts/search.py" "<keyword>" --stack <stack>
```
...
python3 "$PATI_UI_DECIDE/scripts/search.py" "AI search tool modern minimal" --design-system -p "AI Search"

# Step 3: supplement
python3 "$PATI_UI_DECIDE/scripts/search.py" "keyboard focus modal" --domain ux

# Step 4: stack guidelines
python3 "$PATI_UI_DECIDE/scripts/search.py" "suspense streaming bundle" --stack nextjs
```

Then synthesize the design system and detailed searches into an implementation
handoff. If the task also requests code changes, apply `pk-ui-verify` during
implementation and complete its live-browser checks before reporting done.
...
## Absorbed sibling skills (references/)

This skill also carries four smaller design skills, kept verbatim as
reference files — read the relevant one on demand, do not load all four by
default:

- `references/frontend-design.md` — distinctive aesthetic direction, typography, anti-templated-defaults process (Anthropic, adapted). Use when the ask is "make this look distinctive / not generic AI slop" for a new page or redesign.
- `references/design-taste-frontend.md` — anti-slop checklist specifically for landing pages, portfolios, and redesigns; brief-inference first, contextual rules only.
- `references/web-design-guidelines.md` — fetches and applies the live Vercel Web Interface Guidelines against a diff/file set. Use for a compliance review pass, not for greenfield design.
- `references/accessibility.md` — WCAG 2.2 AA implementation reference (POUR, ARIA, cross-platform Web/iOS/Android mapping). Use alongside the Accessibility rule category above when a task needs concrete ARIA/semantic-HTML patterns, not just the priority checklist.

Route: new/redesigned UI needing a point of view → `frontend-design.md` or
`design-taste-frontend.md` first, then the searchable data above for a
concrete design system. Reviewing existing UI → `web-design-guidelines.md` +
`accessibility.md`. Live-browser verification of any UI change (not
aesthetic judgment) → the sibling skill `pk-ui-verify`, not this one.
