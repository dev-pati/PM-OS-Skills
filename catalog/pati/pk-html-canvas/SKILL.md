...
**Reference implementation:** `05_handoffs/internal/new-ads-launcher/specs/account-credential-infrastructure/account-infra-canvas.html` (full) and `account-infra-onepager.html` (1â€“2 page CEO scan). Prefer copying patterns from those over inventing new chrome.

## When to use which branch

| Signal | Branch | Length |
|---|---|---|
| CEO / MM / "quick scan" / "1â€“2 page" / non-technical | **One-pager** | 1â€“2 screenfuls |
| Architecture review, migration, options, DB sign-off, multi-section proposal | **Full canvas** | Numbered sections, as long as needed |
| Both | One-pager + full canvas, same folder, linked | â€” |

Default language = user's language. Code identifiers, table/column names, SQL, file paths stay English.

## Hard rules

1. **Evidence over assertion.** Numbers, counts, FK names, route names come from code/DB/docs. If unknown, say unknown â€” never invent KPIs or row counts.
2. **Options before recommendation.** When the page chooses among designs: show â‰¥2 real alternatives with pros/cons/cost first; verdict after. No rigged matrix that only A wins.
3. **Diagrams carry the claim.** Dual-lane flows, before/after trees, 3-panel option SVGs, ERDs â€” not walls of prose. Every critical distinction gets an SVG or table the eye can scan in 10s.
4. **App tokens only.** Colors, radius, shadows, type from Star Admin 2 Pro (`02_projects/adlauncher/app/globals.css`). No JetBrains Mono dark cyber theme (that is `/pk-architecture-diagram`). No generic SaaS purple unrelated to the app.
...
6. **Obsidian-readable location.** Write under `specs/<feature-slug>/`, never `07_temp/scratch/` (hidden) and never `src/` (wrong audience). Pair with a sibling `.md` only when the user asked for a durable AS-IS/spec write â€” the HTML is the scan surface.
7. **SVG rethemes.** Diagram fills/strokes/text use `var(--â€¦)` so light/dark both work. No hard-coded `#1f3bb3` inside SVG attributes except when a marker fill must resolve at paint time via CSS `fill="var(--rose)"`.
...
- Live counts / schema / FKs from Supabase or `01_brain/07_temp/scratch/<project>/schema-dump-*` (e.g. `schema-dump-ads-launcher-20260810` under `07_temp/scratch/new-ads-launcher/`)
...
2. 3â€“4 stats (real numbers only)
3. Before â†’ after SVG
...
3. For options: three `.opt` cards â†’ comparison SVG or de-rigged matrix â†’ `.verdict` with why + honest counter-argument + flip conditions.
...
Canonical source: `02_projects/adlauncher/app/globals.css`. Snapshot used by canvases:
...
```html
<button class="themebtn" onclick="document.documentElement.classList.toggle('dark')">â— Light / Dark</button>
```

Full chrome (header, cards, tables, badges, tree, opts, SVG classes, callouts, steps): load [`resources/chrome.css`](resources/chrome.css) into the page's `<style>` block â€” do not link it as an external file at runtime; paste/inline so the HTML stays self-contained.
...
SVG hygiene:
- `viewBox` set; `min-width` on `svg.dg` (~900â€“1100) so it scrolls inside `.dgwrap` on narrow panes
...
1. Heading: alternatives first ("Ba phÆ°Æ¡ng Ã¡n â€” Ä‘áº·t cáº¡nh nhau trÆ°á»›c khi chá»n")
2. Visual compare (3-panel SVG or honest matrix â€” include rows where the recommended option loses)
3. Three `.opt` cards: what / pro / con / cost
4. Callout correcting any prior misleading claim if this is a revision
5. `.verdict`: pick + 2â€“3 reasons + strongest counter-argument + flip-to-other conditions
...
| Need | Use instead |
|---|---|
| Production app UI | Code in `02_projects/adlauncher/` |
| Dark cyber infra topology | `/pk-architecture-diagram` |
...
Page opens as `file://`, answers the decision it was built for, every claim is sourced or marked unknown, light/dark both work, and â€” when options exist â€” the recommendation is impossible to read without first seeing the alternatives.
